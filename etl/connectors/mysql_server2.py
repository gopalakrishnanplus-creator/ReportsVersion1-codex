...
def extract_table(table: str) -> list[dict[str, Any]]:
    """Extract an entire source table from MySQL server 2."""
    try:
        with pymysql.connect(**_connection_params(settings.MYSQL_SERVER_2)) as conn:
            with conn.cursor() as cur:
                cur.execute(f"SELECT * FROM `{table}`")
                return list(cur.fetchall())
    except Exception as exc:
        host = settings.MYSQL_SERVER_2.get("HOST")
        port = settings.MYSQL_SERVER_2.get("PORT")
        user = settings.MYSQL_SERVER_2.get("USER")
        database = settings.MYSQL_SERVER_2.get("DATABASE")
        hint = (
            "Check host/port reachability, VPC/security-group access,"
            " credentials, and SSL settings (MYSQL_SERVER2_SSL_MODE / MYSQL_SERVER2_SSL_CA)."
        )
        raise MySQLExtractionError(
            f"mysql_server_2 extract failed for table '{table}' on {host}:{port} "
