...
class PortalMySQLExtractionError(RuntimeError):
    """Raised when PE portal extraction fails."""


def _connection_params() -> dict[str, Any]:
    server_settings = settings.PE_PORTAL_MYSQL
    params: dict[str, Any] = {
...
def extract_rows(table: str, columns: list[str], watermark_field: str | None = None, watermark_start: str | None = None) -> list[dict[str, Any]]:
    try:
        ...
        with pymysql.connect(**_connection_params()) as conn:
            with conn.cursor() as cursor:
                ...
                if not selected_columns:
                    raise PortalMySQLExtractionError(f"None of the requested columns exist on table '{table}'")
        ...
        cfg = settings.PE_PORTAL_MYSQL
        raise PortalMySQLExtractionError(
            f"PE portal extract failed for table '{table}' on {cfg.get('HOST')}:{cfg.get('PORT')} "
