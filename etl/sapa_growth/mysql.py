...
class SapaMySQLExtractionError(RuntimeError):
    """Raised when SAPA source extraction from MySQL fails."""


def _connection_params() -> dict[str, Any]:
    server_settings = settings.SAPA_MYSQL
    params: dict[str, Any] = {
...
def extract_rows(table: str, columns: list[str], watermark_field: str | None = None, watermark_start: str | None = None) -> list[dict[str, Any]]:
    backend = clean_text(getattr(settings, "SAPA_SOURCE_EXTRACTOR_BACKEND", "mysql")).lower() or "mysql"
    if backend != "mysql":
        raise SapaMySQLExtractionError(
            f"SAPA MySQL extraction only supports backend 'mysql'. Received '{backend}'. "
    ...
    try:
        ...
        cfg = settings.SAPA_MYSQL
        raise SapaMySQLExtractionError(
            f"SAPA MySQL extract failed for table '{table}' on {cfg.get('HOST')}:{cfg.get('PORT')} "
